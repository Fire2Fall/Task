...
function addCol() {
  ...
  for (var i = 0; i < row.length; i++) {
    var element = row[i];
    element.insertAdjacentHTML('beforeend', '<div class="col" onmouseover="showCol(this)"></div>');
  }
...
function showRow(e) {
  for (i = 0; i < row.length; i++){
    if (row[i] == e){
      for (var el = 0; el < delRows.length; el++) {
        if (el == i) {
          delRows[i].setAttribute('style', 'visibility: visible;');
        } else {
          delRows[el].setAttribute('style', 'visibility: hidden;');
        }
        if (delRows.length == 1) {
